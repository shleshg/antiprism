...
class DatabaseModel {
	...
	validateGetParameter(param) {
		...
		for (let prop in this._model.fields) {
			if (Object.prototype.hasOwnProperty.call(this._model.fields, prop)) {
				if (prop === value) {
					return true;
				}
			}
	...
	validateSetParameter(name, value) {
		assert.ok(typeof name === 'string', 'set parameter unknown type');
		for (let prop in this._model.fields) {
			if (Object.prototype.hasOwnProperty.call(this._model.fields, prop)) {
				if (prop === name) {
					const typeName = this._model.fields[name].typeName.toLowerCase();
					if (typeName === 'int') {
